...
export const sendPeriodicJobListings = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        while (currentDays < totalDays) {
            await step.sleep("wait-interval", "24h");
            ...
            if (recentJobs.length > 0) {
                ...
                try {
                    const result = await resend.emails.send({
                        ...
                        to: ['tanvipatel5176@gmail.com'],
                        subject: 'Latest Job Opportunities for you',
                        html : `
                        <div style="font-family: Arial . sans-serif ; max-width: 600px; margin: 0 auto;">
                        <h2>Letest Job Opportunities</h2>
                        ${jobListingsHtml}
                    ...
                    console.log(`[Inngest] Email sent to ${email}:`, result);
                } catch (error) {
                    console.error(`[Inngest] Failed to send email to ${email}`, error);
                    throw error; // Ensure Inngest tracks the failure
                }
            } )
            }
        }
...
//         while(currentDays < totalDays) {
//             await step.sleep("wait-interval", `10s`)
//             currentDays += intervaldays;
